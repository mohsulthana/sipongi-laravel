...
class ImportHotspotLapanNew implements ToCollection, WithChunkReading, WithStartRow
{
    ...
    public function collection(Collection $rows)
    {
        ...
        $insertData = array();
        foreach ($rows as $col) {
            if (!$col->filter()->isEmpty() && !empty($col[11])) { // check column row not empty and total column harus lebih dari 7
                ...
                if ($count <= 0) {
                    ...
                    $provinsi = $fungsiKawasan ? $fungsiKawasan->provinsi : null;
