...
class ChunkSumKonsesi implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        DB::table('sum_konsesi')->insertOrIgnore($data);
        DB::delete(
            'DELETE FROM sum_konsesi WHERE id IN ( 
                SELECT id FROM ( 
                    SELECT id, row_number() OVER w as rnum FROM sum_konsesi 
                    WINDOW w AS ( 
    ...
    {
        return ['migrasi', 'migrasi:sum-konsesi', 'sum-konsesi:insert-data'];
    }
