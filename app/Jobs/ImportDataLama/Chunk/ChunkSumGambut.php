...
class ChunkSumGambut implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        DB::table('sum_gambut')->insertOrIgnore($data);
        DB::delete(
            'DELETE FROM sum_gambut WHERE id IN ( 
                SELECT id FROM ( 
                    SELECT id, row_number() OVER w as rnum FROM sum_gambut 
                    WINDOW w AS ( 
    ...
    {
        return ['migrasi', 'migrasi:sum-gambut', 'sum-gambut:insert-data'];
    }
