...
class ChunkSumPipib implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        DB::table('sum_pipib')->insertOrIgnore($data);
        DB::delete(
            'DELETE FROM sum_pipib WHERE id IN ( 
                SELECT id FROM ( 
                    SELECT id, row_number() OVER w as rnum FROM sum_pipib 
                    WINDOW w AS ( 
    ...
    {
        return ['migrasi', 'migrasi:sum-pipib', 'sum-pipib:insert-data'];
    }
