...
class ChunkSumLhnKritis implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        DB::table('sum_lhn_kritis')->insertOrIgnore($data);
        DB::delete(
            'DELETE FROM sum_lhn_kritis WHERE id IN ( 
                SELECT id FROM ( 
                    SELECT id, row_number() OVER w as rnum FROM sum_lhn_kritis 
                    WINDOW w AS ( 
    ...
    {
        return ['migrasi', 'migrasi:sum-lhn-kritis', 'sum-lhn-kritis:insert-data'];
    }
