...
class ChunkSumDeforestasi implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        DB::table('sum_deforestasi')->insertOrIgnore($data);
        DB::delete(
            'DELETE FROM sum_deforestasi WHERE id IN ( 
                SELECT id FROM ( 
                    SELECT id, row_number() OVER w as rnum FROM sum_deforestasi 
                    WINDOW w AS ( 
                        PARTITION BY provinsi_id, provinsi, kotakab_id, kecamatan_id, kecamatan, kelurahan_id, desa, nama, jenis, tahun, kml, sumber ORDER BY id 
                    )
                ) t 
            WHERE t.rnum > 1)'
        );
    ...
    {
        return ['migrasi', 'migrasi:sum-deforestasi', 'sum-deforestasi:insert-data'];
    }
