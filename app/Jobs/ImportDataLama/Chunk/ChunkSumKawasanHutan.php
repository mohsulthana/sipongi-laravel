...
class ChunkSumKawasanHutan implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        DB::table('sum_kawasan_hutan')->insertOrIgnore($data);
        DB::delete(
            'DELETE FROM sum_kawasan_hutan WHERE id IN ( 
                SELECT id FROM ( 
                    SELECT id, row_number() OVER w as rnum FROM sum_kawasan_hutan 
                    WINDOW w AS ( 
    ...
    {
        return ['migrasi', 'migrasi:sum-kawasan-hutan', 'sum-kawasan-hutan:insert-data'];
    }
