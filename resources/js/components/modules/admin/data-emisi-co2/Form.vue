        ...
        <v-row>
          ...
          >
            <v-text-field
              ...
              :readonly="showDetail"
              hide-details="auto"
              :error="dataForm.errors.has('total')"
              :error-messages="dataForm.errors.get('total')"
              :disabled="dataForm.processing"
              autocomplete="off"
              @input="clearError('total')"
            >
              <template v-slot:label>
                Total<span class="red--text">*</span>
              </template>
            </v-text-field>
          </v-col>
    ...
    title() {
      return this.id !== '0'
        ? this.showDetail
          ...
          : 'Ubah Emisi CO2 Tahunan'
        : 'Tambah Emisi CO2 Tahunan'
    },
    submitUrl() {
      return this.id !== '0'
        ? this.$api.path('emisiCo2.update', {
            id: this.id,
          })
        : this.$api.path('emisiCo2')
    },
    method() {
      ...
      await this.$http
        ...
        .then(async ({ data }) => {
          ...
          if (data.status === true) {
            ...
            this.$router.push({ name: 'emisi-co2' })
          }
        })
        .catch((error) => {
          this.dataForm.successful = false
