...
export default {
  ...
  methods: {
    ...
    async getLists() {
      ...
      await this.$http
        .get(this.$api.path('luasKebakaran'), {
          params: params,
        })
        .then(({ data }) => {
    ...
    deleteItem() {
      ...
      this.$swal({
        ...
        if (result.value) {
          this.$http
            .post(this.$api.path('luasKebakaran.delete'),{deleteSelected: self.selected})
            .then(async ({ data }) => {
              if (data.status) {
                await self.getLists()
