...
export default {
  ...
  methods: {
    ...
    async getLists() {
      ...
      // http://127.0.0.1:8081/api/running-text
      // this.$api.path('runningText')
      await this.$http
        .get(this.$api.path('runningText'), {
          params: params,
        })
        .then(({ data }) => {
    ...
    deleteItem() {
      ...
      this.$swal({
        ...
        if (result.value) {
          //'http://127.0.0.1:8081/api/running-text/delete',{deleteSelected: self.selected}
          this.$http
            .post(this.$api.path('runningText.delete'),{deleteSelected: self.selected})
            .then(async ({ data }) => {
              if (data.status) {
                await self.getLists()
