    ...
    <v-card-text>
      <v-row>
        <v-col cols="12" class="text-right">
          <v-btn
            v-if="$can('delete-laporan_harian') && selected.length > 0"
            dark
            color="red darken-4"
            @click="deleteItem()"
          ...
          />
          <v-btn
            :to="{ name: 'struktur-organisasi-create' }"
            color="primary"
      ...
      <v-card outlined>
        <v-card-text class="pa-0">
          <v-data-table
            ...
            :show-select="$can('delete-struktur_organisasi')"
            fixed-header
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <span class="text-no-wrap">
                <v-tooltip v-if="$can('update-struktur_organisasi')" top>
...
export default {
  ...
  methods: {
    ...
    async getLists() {
      ...
      // 'http://127.0.0.1:8081/api/struktur-organisasi'
      await this.$http
        .get(this.$api.path('sturkturOrganisasi'), {
          params: params,
        })
        .then(({ data }) => {
    ...
    deleteItem() {
      ...
      this.$swal({
        ...
        if (result.value) {
          // this.$api.path('sturkturOrganisasi.delete')
          // 'http://127.0.0.1:8081/api/struktur-organisasi/delete'
          this.$http
            .post(this.$api.path('sturkturOrganisasi.delete'),{deleteSelected: self.selected})
            .then(async ({ data }) => {
              if (data.status) {
                await self.getLists()
