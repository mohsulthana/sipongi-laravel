...
export default {
  ...
  computed: {
    ...
    submitUrl() {
        return this.id !== '0'
          ? this.$api.path('sarpras.update', {
              id: this.id,
            })
          : this.$api.path('sarpras')
      // return this.id !== '0'
      //   ? 'http://127.0.0.1:8081/api/manggala-agni/sarpras/' + this.id + '/update'
      //   : 'http://127.0.0.1:8081/api/manggala-agni/sarpras/store'
    },
    rules() {
      return [
        (value) =>
  ...
  methods: {
    async loadData() {
      // await this.$http
      //   .get('http://127.0.0.1:8081/api/manggala-agni/sarpras/' + this.id + '/edit')
      //   .then(({ data }) => {
