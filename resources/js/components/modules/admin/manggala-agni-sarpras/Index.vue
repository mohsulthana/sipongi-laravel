  ...
  <v-card outlined>
    <v-toolbar flat color="primary" dark>
      <v-toolbar-title class="font-weight-black">
        Manggala Agni - Sarana & Prasarana
      </v-toolbar-title>
    </v-toolbar>
    <v-card-text>
      <v-row>
      ...
      <v-card outlined>
        <v-card-text class="pa-0">
          <v-data-table
            ...
            :show-select="$can('delete-sarpras')"
            fixed-header
...
export default {
  breadcrumb() {
    return [
      ...
      {
        text: 'Manggala Agni Sarana & Prasarana',
        to: { name: 'sarpras' },
      },
    ]
  },
  ...
  methods: {
    ...
    async getLists() {
      ...
      // this.$api.path('sarpras')
      // 'http://127.0.0.1:8081/api/manggala-agni/sarpras'
      await this.$http
        .get(this.$api.path('sarpras'), {
          params: params,
        })
        .then(({ data }) => {
    ...
    deleteItem() {
      ...
      this.$swal({
        ...
        if (result.value) {
          // this.$api.path('sarpras')
          // 'http://127.0.0.1:8081/api/manggala-agni/sarpras/delete'
          this.$http
            .post(this.$api.path('sarpras.delete'),{deleteSelected: self.selected})
            .then(async ({ data }) => {
              if (data.status) {
                await self.getLists()
