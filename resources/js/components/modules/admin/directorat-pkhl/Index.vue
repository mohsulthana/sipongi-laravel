    ...
    <v-card-text>
      <v-row>
        <v-col cols="12" class="text-right">
          ...
          />
          <v-btn
            :to="{ name: 'direktorat-pkhl-create' }"
            color="primary"
        ...
        <v-card-text class="pa-0">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <span class="text-no-wrap">
                <v-tooltip v-if="$can('update-direktorat-pkhl')" top>
...
export default {
  ...
  methods: {
    ...
    async getLists() {
      ...
      // 'http://127.0.0.1:8081/api/direktorat-pkhl'
      await this.$http
        .get(this.$api.path('direktoratPKHL'), {
          params: params,
        })
        .then(({ data }) => {
    ...
    deleteItem() {
      ...
      this.$swal({
        ...
        if (result.value) {
          ...
          // 'http://127.0.0.1:8081/api/direktorat-pkhl/delete'
          this.$http
            .post(this.$api.path('direktoratPKHL.delete'),{deleteSelected: self.selected})
            .then(async ({ data }) => {
              if (data.status) {
                await self.getLists()
