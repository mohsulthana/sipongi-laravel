        ...
        <v-row>
          ...
          >
            <v-file-input
              ...
              hint="File hanya boleh ( DOCX | PPTX | XSLX | PDF ) dan ukuran file harus kurang dari 20 MB!"
              persistent-hint
          ...
          <v-col cols="12" class="pa-2" md="6">
            <v-switch
              ...
              :readonly="showDetail"
              hide-details="auto"
              :error="dataForm.errors.has('private')"
              :error-messages="dataForm.errors.get('private')"
              :disabled="dataForm.processing"
            >
              <template v-slot:label>
                Private
    ...
    <v-card-actions>
      <v-row>
        <v-col cols="12" class="text-right">
          ...
          />
          <v-btn
            v-if="!showDetail && $can('update-dokumen_lain')"
            color="primary"
            :loading="dataForm.processing"
            :disabled="dataForm.processing"
            @click="submit"
...
export default {
  ...
  computed: {
    ...
    title() {
      return this.id !== '0'
        ? this.showDetail
          ? 'Detail Data Dokumen Lain'
          : 'Ubah Data Dokumen Lain'
        : 'Tambah Data Dokumen Lain'
    },
    submitUrl() {
      return this.id !== '0'
        ? this.$api.path('dokumenLain.update', {
            id: this.id,
          })
        : this.$api.path('dokumenLain')
    },
    rules() {
      return [
        (value) =>
  ...
  methods: {
    ...
    async loadData() {
      await this.$http
        .get(
          this.$api.path('dokumenLain.edit', {
            id: this.id,
          })
        )
        .then(({ data }) => {
      ...
      await this.$http
        ...
        .then(async ({ data }) => {
          ...
          if (data.status === true) {
            ...
            this.$router.push({ name: 'dokumen_lain' })
          }
        })
        .catch((error) => {
          this.dataForm.successful = false
