    ...
    <v-card-text>
      <v-row>
        <v-col cols="12" class="text-right">
          <v-btn
            v-if="$can('delete-dokumen_lain') && selected.length > 0"
            dark
            color="red darken-4"
            @click="deleteItem()"
          ...
          />
          <v-btn
            v-if="$can('create-dokumen_lain')"
            :to="{ name: 'dokumen_lain-create' }"
            color="primary"
      ...
      <v-card outlined>
        <v-card-text class="pa-0">
          <v-data-table
            ...
            :show-select="$can('delete-dokumen_lain')"
            fixed-header
          ...
          >
            ...
            <template v-slot:item.pub_dokumen_lain.title="{ item }">
              <span class="text-no-wrap">
                {{ item.title }}
              </span>
            </template>
            <template v-slot:item.pub_dokumen_lain.private="{ item }">
              <span class="text-no-wrap">
                {{ item.private ? 'Ya' : 'Tidak' }}
              </span>
            </template>
            <template v-slot:item.pub_dokumen_lain.file_url="{ item }">
            ...
            <template v-slot:item.pub_dokumen_lain.created_at="{ item }">
              <span class="text-no-wrap">
            ...
            <template v-slot:item.actions="{ item }">
              <span class="text-no-wrap">
                ...
                <v-tooltip v-if="$can('update-dokumen_lain')" top>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      depressed
                      outlined
...
export default {
  ...
  data: () => ({
    ...
    options: {
      ...
      page: 1,
      itemsPerPage: 10,
      sortBy: ['pub_dokumen_lain.created_at'],
    },
    footerProps: {
      ...
      return [
        {
          ...
          align: 'start',
          class: 'text-no-wrap',
          value: 'pub_dokumen_lain.title',
        },
        {
          text: 'Private',
          align: 'start',
          class: 'text-no-wrap',
          value: 'pub_dokumen_lain.private',
        },
        {
          text: 'File/URL',
          align: 'start',
          class: 'text-no-wrap',
          value: 'pub_dokumen_lain.file_url',
        },
        {
          text: 'Dibuat',
          align: 'start',
          class: 'text-no-wrap',
          value: 'pub_dokumen_lain.created_at',
        },
        {
          text: 'Aksi',
          align: 'start',
  ...
  methods: {
    ...
    deleteItem() {
      ...
      this.$swal({
        ...
        if (result.value) {
          this.$http
            .post(this.$api.path('dokumenLain.delete'), {
