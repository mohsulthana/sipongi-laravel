    ...
    <v-card-text>
      <v-row>
        <v-col cols="12" class="text-right">
          ...
          />
          <v-btn
            :to="{ name: 'data-laporan-harian-create' }"
            color="primary"
      ...
      <v-card outlined>
        <v-card-text class="pa-0">
          <v-data-table
            ...
            :show-select="$can('delete-data_laporan_harian')"
            fixed-header
...
export default {
  ...
  methods: {
    ...
    async getLists() {
      ...
      // 'http://127.0.0.1:8081/api/laporan-harian'
      await this.$http
        .get(this.$api.path('laporanHarian'), {
          params: params,
        })
        .then(({ data }) => {
    ...
    deleteItem() {
      ...
      this.$swal({
        ...
        if (result.value) {
          // 'http://127.0.0.1:8081/api/laporan-harian/delete'
          this.$http
            .post(this.$api.path('laporanHarian.delete'),{deleteSelected: self.selected})
            .then(async ({ data }) => {
              if (data.status) {
                await self.getLists()
