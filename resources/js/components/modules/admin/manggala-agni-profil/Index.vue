      ...
      <v-card outlined>
        <v-card-text class="pa-0">
          ...
          >
            ...
            <template v-slot:item.urutan="{ item }">
              <span class="text-no-wrap">
                {{ item.urutan }}
              </span>
            </template>
...
export default {
  ...
  methods: {
    ...
    async getLists() {
      ...
      // 'http://127.0.0.1:8081/api/manggala-agni/profil'
      await this.$http
        .get(this.$api.path('profil'), {
          params: params,
        })
        .then(({ data }) => {
    ...
    deleteItem() {
      ...
      this.$swal({
        ...
        if (result.value) {
          // this.$api.path('profil')
          // http://127.0.0.1:8081/api/manggala-agni/profil/delete'
          this.$http
            .post(this.$api.path('profil.delete'),{deleteSelected: self.selected})
            .then(async ({ data }) => {
              if (data.status) {
                await self.getLists()
